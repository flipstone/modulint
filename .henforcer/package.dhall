...
let
    minimumDocumentedAndMaxUndocumented =
      \(min : Natural) ->
      \(max : Natural) ->
        DocumentationRule.Both ({ maximumUndocumented = max
                               , minimumDocumented = min
                               })
...
let DocumentationRules =
    < GlobalDocumentationRule : DocumentationRule
    | PerModuleDocumentationRules : PerModuleDocumentationRules
    >

let perModuleDocumentationRules =
    DocumentationRules.PerModuleDocumentationRules

let globalModuleDocumentationRule =
    DocumentationRules.GlobalDocumentationRule
...
in  { Config =
      ...
      , default =
        ...
        , documentationRules = perModuleDocumentationRules (toMap {=} : PerModuleDocumentationRules)
        , allowedOpenUnaliasedImports =
            AllowedOpenUnaliasedImports.PerModuleOpenUnaliasedImports (toMap {=} : OpenUnaliasedImportMap)
        }
      }
    , Qualification
    , Alias
